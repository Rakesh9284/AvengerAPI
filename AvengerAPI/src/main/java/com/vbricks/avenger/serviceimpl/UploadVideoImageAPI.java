...
public class UploadVideoImageAPI {
	 ...
	 *         
	 */
	public HashMap<String, String> uploadVideoImageFile(HashMap<String, String> loginapiresponse, String path, String inputJson) {

		String projectDir = System.getProperty("user.dir");
		String requestURL = loginapiresponse.get("baseurl") + RevbaseAPIURLs.IMAGEFILES+loginapiresponse.get("videoId");
		logger.info("loginapiresponse API URL :::: " + loginapiresponse.get("baseurl"));
		logger.info("UPLOADVIDEOURL   URL :::: " + RevbaseAPIURLs.IMAGEFILES);
		...
		try {
			...
			multipart.addFilePart("SFile", uploadFile,inputJson);
			...
			if (response.get(0).equals("HTTP StatusCode :200")) {
				...
				if (inputJson.equalsIgnoreCase("video")) {
					logger.info("UploadVideo API HTTP status Code ::: "+response.get(0));
				...
				else if(inputJson.equalsIgnoreCase("transcription")){
					logger.info("UploadVideo API HTTP status Code ::: "+response.get(0));
				 	logger.info("Video Upload Response ..@@@@@@::::");
						for (String line : response)  
							logger.info("Video uploaed API response :::" + line);
				...
				}else if(inputJson.equalsIgnoreCase("supplemental")){
					logger.info("UploadVideo API HTTP status Code ::: "+response.get(0));
