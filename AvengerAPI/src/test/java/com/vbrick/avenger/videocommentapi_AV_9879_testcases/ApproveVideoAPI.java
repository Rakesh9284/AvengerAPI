...
public class ApproveVideoAPI extends TestBase{

	private static Logger logger = Logger.getLogger(ApproveVideoAPI.class);
	...
	@Test(description="To Verify the ApproveVideopendingforapproval using ApproveVideopendingforapproval API with MediaContributor",groups = {APPROVEVIDEO})
	public void TC_01_PUT_ApproveVideopendingforapproval_api_check_With_MediaContributor_Positive(ITestContext context ) {
		
		String approvalid=create_ApprovalProcessFromUI(IUsersList.ApproveVideomediacontributor);
		...
		loginapiresponse_mediacontributor = userservices.doLogin(apiutils.userJson(IUsersList.ApproveVideomediacontributor), surl);
		...
		ApproveVideo approveVideo=new ApproveVideo();
		JSONObject Approvaljson1=approveVideo.createApprovalJson();
		sendvideoapprovalapirespone = approveVideo.approveVideo(loginapiresponse_mediacontributor,Approvaljson1);
		assertionapiresponse.verifyAssert_httpCode(sendvideoapprovalapirespone.get(IAPIConstantCodes.APIResponseHttpCode)+sendvideoapprovalapirespone.get(IAPIConstantCodes.APIResponseStatusInfo), HttpStatusCode.httpsStatusCode200OK);
		customReport.customizedReport(HttpStatusCode.httpsStatusCode200OK, sendvideoapprovalapirespone.get(IAPIConstantCodes.APIResponseHttpCode)+sendvideoapprovalapirespone.get(IAPIConstantCodes.APIResponseStatusInfo),statusValue, driver, sTestcaseName);
		...
		homePage = loginPage.loginAs(IUsersList.ApproveVideomediacontributor, sPassword);
		...
		customReport.customizedReport(0,commentspage.approveRequestButtonAvailability(), statusValue, driver, sTestcaseName);
		customReport.checkinglist(statusValue);
	}
	 
	@Test(description="To Verify the ApproveVideopendingforapproval using ApproveVideopendingforapproval API with MediaViewer",groups = {APPROVEVIDEO})
	public void TC_02_PUT_ApproveVideopendingforapproval_api_check_With_MediaViewer_Positive(ITestContext context ) {
		
		String approvalid=create_ApprovalProcessFromUI(IUsersList.ApproveVideomediaviewer);
		logger.info("API Level Code is excuting");
		sTestcaseName = new Object() {}.getClass().getEnclosingMethod().getName();
		...
		loginapiresponse_mediaviewer = userservices.doLogin(apiutils.userJson(IUsersList.ApproveVideomediaviewer), surl);
		...
		ApproveVideo approveVideo=new ApproveVideo();
		JSONObject Approvaljson1=approveVideo.createApprovalJson();
		sendvideoapprovalapirespone = approveVideo.approveVideo(loginapiresponse_mediaviewer,Approvaljson1);
		assertionapiresponse.verifyAssert_httpCode(sendvideoapprovalapirespone.get(IAPIConstantCodes.APIResponseHttpCode)+sendvideoapprovalapirespone.get(IAPIConstantCodes.APIResponseStatusInfo), HttpStatusCode.httpsStatusCode200OK);
		customReport.customizedReport(HttpStatusCode.httpsStatusCode200OK, sendvideoapprovalapirespone.get(IAPIConstantCodes.APIResponseHttpCode)+sendvideoapprovalapirespone.get(IAPIConstantCodes.APIResponseStatusInfo),statusValue, driver, sTestcaseName);
		...
		homePage = loginPage.loginAs(IUsersList.ApproveVideomediaviewer, sPassword);
		AvengerDashboardPage avengerdashboardpage = homePage.avengerDashboardPage();
		avengerdashboardpage.searchMedia(adduploadvideobean.getTitle());
		AvengerVideoCommentsPage commentspage = avengerdashboardpage.clickVideo(adduploadvideobean.getTitle());
		customReport.customizedReport(0,commentspage.approveRequestButtonAvailability(), statusValue, driver, sTestcaseName);
	...
	@Test(description="To Verify the ApproveVideopendingforapproval using ApproveVideopendingforapproval API with Media Admin",groups = {APPROVEVIDEO})
	public void TC_03_PUT_ApproveVideopendingforapproval_api_check_With_MediaAdmin_Positive(ITestContext context ) {
		
		String approvalid=create_ApprovalProcessFromUI(IUsersList.ApproveVideomediaadmin);
		logger.info("API Level Code is excuting");
		sTestcaseName = new Object() {}.getClass().getEnclosingMethod().getName();
		...
		loginapiresponse_mediaadmin = userservices.doLogin(apiutils.userJson(IUsersList.ApproveVideomediaadmin), surl);
		...
		ApproveVideo approveVideo=new ApproveVideo();
		JSONObject Approvaljson1=approveVideo.createApprovalJson();
		sendvideoapprovalapirespone = approveVideo.approveVideo(loginapiresponse_mediaadmin,Approvaljson1);
		assertionapiresponse.verifyAssert_httpCode(sendvideoapprovalapirespone.get(IAPIConstantCodes.APIResponseHttpCode)+sendvideoapprovalapirespone.get(IAPIConstantCodes.APIResponseStatusInfo), HttpStatusCode.httpsStatusCode200OK);
		customReport.customizedReport(HttpStatusCode.httpsStatusCode200OK, sendvideoapprovalapirespone.get(IAPIConstantCodes.APIResponseHttpCode)+sendvideoapprovalapirespone.get(IAPIConstantCodes.APIResponseStatusInfo),statusValue, driver, sTestcaseName);
		...
		customReport.reporter("Application launch with URL : ", surl);
		//login as Media Admin
		homePage = loginPage.loginAs(IUsersList.ApproveVideomediaadmin, sPassword);
		AvengerDashboardPage avengerdashboardpage = homePage.avengerDashboardPage();
		avengerdashboardpage.searchMedia(adduploadvideobean.getTitle());
		AvengerVideoCommentsPage commentspage = avengerdashboardpage.clickVideo(adduploadvideobean.getTitle());
		customReport.customizedReport(0,commentspage.approveRequestButtonAvailability(), statusValue, driver, sTestcaseName);
	...
	@Test(description="To Verify the ApproveVideopendingforapproval using ApproveVideopendingforapproval API with Event Admin",groups = {APPROVEVIDEO})
	public void TC_04_PUT_ApproveVideopendingforapproval_api_check_With_EventAdmin_Positive(ITestContext context ) {
		
		String approvalid=create_ApprovalProcessFromUI(IUsersList.ApproveVideoeventadmin);
		logger.info("API Level Code is excuting");
		sTestcaseName = new Object() {}.getClass().getEnclosingMethod().getName();
		...
		loginapiresponse_eventadmin = userservices.doLogin(apiutils.userJson(IUsersList.ApproveVideoeventadmin), surl);
		...
		ApproveVideo approveVideo=new ApproveVideo();
		JSONObject Approvaljson1=approveVideo.createApprovalJson();
		sendvideoapprovalapirespone = approveVideo.approveVideo(loginapiresponse_eventadmin,Approvaljson1);
		assertionapiresponse.verifyAssert_httpCode(sendvideoapprovalapirespone.get(IAPIConstantCodes.APIResponseHttpCode)+sendvideoapprovalapirespone.get(IAPIConstantCodes.APIResponseStatusInfo), HttpStatusCode.httpsStatusCode200OK);
		customReport.customizedReport(HttpStatusCode.httpsStatusCode200OK, sendvideoapprovalapirespone.get(IAPIConstantCodes.APIResponseHttpCode)+sendvideoapprovalapirespone.get(IAPIConstantCodes.APIResponseStatusInfo),statusValue, driver, sTestcaseName);
		...
		customReport.reporter("Application launch with URL : ", surl);
		//login as Media Admin
		homePage = loginPage.loginAs(IUsersList.ApproveVideoeventadmin, sPassword);
		AvengerDashboardPage avengerdashboardpage = homePage.avengerDashboardPage();
		avengerdashboardpage.searchMedia(adduploadvideobean.getTitle());
		AvengerVideoCommentsPage commentspage = avengerdashboardpage.clickVideo(adduploadvideobean.getTitle());
		customReport.customizedReport(0,commentspage.approveRequestButtonAvailability(), statusValue, driver, sTestcaseName);
