...
public class  DeleteUsers_CleanUp extends TestBase {

	private static Logger logger = Logger.getLogger(DeleteUsers_CleanUp.class);
	private AvengerLoginPage loginPaDeleteUsers_CleanUpge;
	...
	public void TC_01_DELETE_DeleteVideo_api_check_DeleteVideo_with_AccoutAdmin_Positive() throws InterruptedException {
		...
		jse.executeScript("window.scrollTo(0, document.body.scrollHeight)");
		 ...
		 List<WebElement> lists = avengeruserdashboardpage
				.getAllWebElementValues(By.xpath("//a[@ui-sref='portal.admin.user-access.users.edit({userId: user.id})']"));
		logger.info("List size si----" + lists.size());
		ArrayList<String> mylist = new ArrayList<String>();
		for (WebElement el : lists) {
			logger.info("href values ----" + el.getAttribute("href")); 
		...
		for (String href : mylist) {
			...
			logger.info("href values ---is " + href.split("/")[8]);
			videoIdLists.add(href.split("/")[8]);
		} 
		...
		customReport.customizedReport(HttpStatusCode.httpsStatusCode200, loginapiresponse.get("httpCode"), statusValue,
		 ...
		 for (String id : videoIdLists) {
			...
			loginapiresponse.put("userId", id);
			DeleteVideosAPI deleteVideosAPI = new DeleteVideosAPI();
			HashMap<String, String> deleteapirespone = deleteVideosAPI.deleteusers(loginapiresponse);
			assertionapiresponse.verifyAssert_httpCode(
					deleteapirespone.get("httpCode") + deleteapirespone.get("statusInfo"),
					HttpStatusCode.httpsStatusCode200OK);
			customReport.customizedReport(HttpStatusCode.httpsStatusCode200OK,
					deleteapirespone.get("httpCode") + deleteapirespone.get("statusInfo"), statusValue, driver,
					sTestcaseName);
		} 
	}
